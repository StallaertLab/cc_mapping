...
This module provides the SequentialGaussianMixtureModelThresholding class for
performing iterative refinement of categorical labels through multiple sequential
thresholding operations.

Classes:
    SequentialGaussianMixtureModelThresholding: Sequential refinement class
"""
...
class SequentialGaussianMixtureModelThresholding(GaussianMixtureModelBase):
    """
    ...
    Unlike GaussianMixtureModelThresholding which thresholds a single feature once,
    this class allows:
    ...
    Attributes:
        ...
        thresholding_events_key (str): Key in adata.uns for storing all operations.
        gmm_kwargs (Dict): Default GMM kwargs (can be overridden per operation).
    ...
    def __init__(
        ...
        Raises:
            ...
            ValueError: If `thresholding_events_key` is an empty string.
    ...
    def threshold_entire_dataset(
        ...
        around GaussianMixtureModelThresholding that stores results in the
        sequential thresholding framework.
        
        Args:
            feature (str): Feature name to threshold on (must exist in adata.var_names).
            label_obs_save_str (str): New column name in adata.obs for labels.
            n_components (int): Number of GMM components to fit.
            ordered_labels (List[str]): Labels to assign (length = n_components).
            ...
            duplicate_labels (bool, optional): Allow duplicate labels for label collapsing.
                Defaults to False.
            operation_name (str): Required name for tracking this operation.
            layer (Optional[str], optional): Layer to use for data access. 
                If None, uses adata.X. Defaults to None.
            gmm_kwargs (Optional[dict], optional): GMM kwargs for this operation.
                Overrides default if provided. Defaults to None.
            overwrite (bool, optional): If True, allows overwriting an existing operation
                with the same name. Useful for updating n_components or thresholds.
                Defaults to False.
        
        Raises:
            ValueError: If operation_name is None or empty.
            KeyError: If operation_name already exists in .uns and overwrite=False.
            Other exceptions raised by GaussianMixtureModelThresholding.
        ...
        gmm_single = GaussianMixtureModelThresholding(
            adata=self.adata,
            feature=feature,
    ...
    def refine_labels_with_gmm(
        ...
        Args:
            feature (str): Feature to threshold on (e.g., 'Plk1').
            obs_label (str): Obs column to modify in-place (e.g., 'cell_cycle').
            value_to_refine (str): Which label value to refine (e.g., 'G0').
            n_components (int): Number of GMM components to fit.
            ordered_labels (List[str]): New labels to assign (e.g., ['G0_low', 'G0_high']).
            duplicate_labels (bool, optional): Allow duplicate labels for label collapsing.
                Defaults to False.
            operation_name (str): Required name for tracking this operation.
            layer (Optional[str], optional): Layer to use for data access.
    ...
    def refine_labels_with_manual_thresholds(
        ...
        Args:
            feature (str): Feature to threshold on.
            obs_label (str): Obs column to modify in-place.
            value_to_refine (str): Which label value to refine.
            manual_thresholds (List[Union[float, int]]): Threshold values.
                Length must be len(ordered_labels) - 1.
            ordered_labels (List[str]): New labels to assign.
            operation_name (str): Required name for tracking this operation.
            layer (Optional[str], optional): Layer to use for data access.
                If None, uses adata.X. Defaults to None.
            overwrite (bool, optional): If True, allows overwriting an existing operation
                with the same name. Useful for updating thresholds.
                Defaults to False.
    ...
    def _partition_data_by_label(
        ...
        Args:
            obs_label: Obs column name to check.
            value_to_refine: Label value to select.
            feature: Feature name to extract data for.
            layer: Layer to use (None = .X).
        
        Returns:
            mask: Boolean array indicating which cells to refine.
            feature_data: Feature values for those cells (1D array).
        
        Raises:
            ValueError: If value_to_refine is not present in obs_label.
            ValueError: If no cells have the value_to_refine.
        """
                    ...
                    if op_data.get('label_to_refine') == value_to_refine:
                        ...
                        if existing_refined:
                            error_msg += (
                                ...
                                f"  1. Restart from the initialization of the SequentialGaussianMixtureModelThresholding object, or\n"
    ...
    def _assign_labels_from_thresholds(
        ...
        Args:
            data: 1D array of feature values.
            thresholds: List of threshold values (sorted low to high).
            ordered_labels: Labels corresponding to threshold bins.
        
        Returns:
            Array of labels (same length as data).
            
        Raises:
            ValueError: If number of labels doesn't match number of thresholds.
        """
    ...
    def _update_labels_in_place(
        ...
        Args:
            obs_label: Obs column to modify.
            mask: Boolean mask indicating which cells to update.
            new_labels: New label values for masked cells.
        """
    ...
    def _store_refinement_operation(
        ...
        Args:
            ...
            internal_data: Pydantic model with thresholding data.
            obs_label: Which obs column was modified.
            value_to_refine: Which label value was refined.
            operation_type: 'refinement' or 'refinement_manual'.
            layer: Which layer was used (None = .X).
            cell_counts_after_operation: Dictionary of label counts immediately after operation.
        """
    ...
    def return_adata(self) -> ad.AnnData:
        ...
        Example::
            ...
            seq_gmm = SequentialGaussianMixtureModelThresholding(adata)
            seq_gmm.threshold_entire_dataset(...)
            seq_gmm.refine_labels_with_gmm(...)
    ...
    def _get_descendant_labels(
        ...
        Args:
            ...
            original_labels: The labels created by this operation.
    ...
    def plot_feature_distribution_exploratory(
        ...
        Args:
            feature (str): Feature name to plot (must exist in adata.var_names).
            obs_label (Optional[str]): Obs column to use for subsetting. If provided
                with value_to_subset, only plots cells with that label value. If None,
                plots all cells. Defaults to None.
            value_to_subset (Optional[str]): Specific label value to plot. Requires
                obs_label to be specified. If None, plots all cells (or all cells in
                obs_label if provided). Defaults to None.
            layer (Optional[str]): Layer to use for data. If None, uses adata.X.
                Defaults to None.
    ...
    def plot_feature_strip_plot_exploratory(
        ...
        Args:
            ...
            scatter_density (bool): If True, uses density-based coloring. If False,
                uses uniform scatter plot. Defaults to True.
            x_axis_limits (Optional[tuple]): (min, max) for x-axis. Use None for
                data-driven limits. Defaults to None.
            vmax (Optional[Union[int, float]]): Maximum density value for colormap.
                Only used when scatter_density=True. If None, auto-calculated.
                Defaults to None.
    ...
    def plot_hist_distribution_with_boundaries(
        ...
        Args:
            operation_name (str): Name of operation to plot (from .uns keys).
            num_std (int, optional): Number of standard deviations for GMM plotting.
                Defaults to 5.
    ...
    def plot_strip_plot_histogram_with_decision_boundaries(
        ...
        Args:
            ...
            y_axis_limits (Optional[Tuple[float, float]], optional): Y-axis limits (min, max).
            ...
            hist_kwargs (Optional[Dict], optional): Kwargs for histogram (bins, color, etc.).
                Defaults to None.
            strip_plot_kwargs (Optional[Dict], optional): Kwargs for strip plot scatter 
