...
def random_forest_feature_selection(adata: ad.AnnData,
    ...
    if re.search('(?<=RF_min_)[0-9]+',method):
        ...
        max_acc_arg = 0
        acc_list = [0]
        for num_feats in tqdm(range(1, sorted_features.shape[1]+1),total= sorted_features.shape[1],
                              desc = f'Training RF model iteratively using most important RF features until {stable_counter} stable iterations', disable= not verbose):
...
class GMM_CC_phase_prediction():
    ...
    def __init__(self,
        ...
        if 'random_state' not in self.GMM_kwargs.keys():
            raise ValueError('The random_state parameter is required for reproducibility.')
    ...
    def row_data_partitioning(self,
            ...
            if GMM_set_name_to_partition is None:
                ...
                gene_adata = row_data_partitioning(gene_adata, obs_search_term, phase_obs_label)
    ...
    def compare_GMM_labels(self, 
            ...
            gene_adata.obs[self.GMM_obs_label] = np.repeat(self.default_label,gene_adata.shape[0])
