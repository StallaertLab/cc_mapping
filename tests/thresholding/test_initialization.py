...
def test_init_success_existing_uns_key(sample_adata):
    ...
    existing_uns_data = {
        "gmm_thresholding_events": OrderedDict({"previous_run": "example_run_1"})
    }  # Example data to simulate a previous run
    modified_adata = create_modified_adata(
        sample_adata, add_uns=existing_uns_data
    )  # Ensure the original sample_adata is unchanged

    feature_name = "gene1"
    label_name = "new_labels"

    gmm_thresholding = GMMThresholding(
        adata=modified_adata, feature=feature_name, label_obs_save_str=label_name
    )
    ...
    assert (
        gmm_thresholding.adata.uns["gmm_thresholding_events"]
        == existing_uns_data["gmm_thresholding_events"]
    ), (
...
@pytest.mark.parametrize("invalid_adata", [None, 123, "string", {}, []])
...
def test_init_existing_uns_key_kept_as_plain_dict(sample_adata, container):
    ...
    existing_adata = create_modified_adata(
        sample_adata,
        add_uns={
            "gmm_thresholding_events": container(previous_run={"feature_name": "gene2"})
        },
    )

    gmm_thresholding = GMMThresholding(
        adata=existing_adata, feature="gene1", label_obs_save_str="labels"
    )

    events = gmm_thresholding.adata.uns["gmm_thresholding_events"]
    assert type(events) is dict, (
        "The thresholding events should be stored in a plain dict."
    )
    assert events == {"previous_run": {"feature_name": "gene2"}}, (
        "The existing events should be preserved."
    )


def test_init_existing_uns_key_wrong_type(sample_adata):
    """Tests TypeError when the uns key exists but is not a dict."""
    wrong_uns_type_adata = create_modified_adata(
        sample_adata, add_uns={"gmm_thresholding_events": ["not", "a", "dict"]}
    )  # Ensure the original sample_adata is unchanged

    # Match the updated error message in __init__
    with pytest.raises(
        TypeError,
        match="The 'gmm_thresholding_events' key in the AnnData object's `.uns` attribute must be a dict.",
    ):
        GMMThresholding(
            adata=wrong_uns_type_adata, feature="gene1", label_obs_save_str="labels"
        )


@pytest.mark.parametrize(
    "invalid_feature, expected_exception, match_pattern",
    [
...
def test_init_invalid_feature_type_or_value(
    sample_adata, invalid_feature, expected_exception, match_pattern
):
    """Tests errors for invalid feature types or empty string."""
    with pytest.raises(expected_exception, match=match_pattern):
        GMMThresholding(
            adata=sample_adata, feature=invalid_feature, label_obs_save_str="labels"
        )
...
def test_init_feature_not_found(sample_adata):
    ...
    with pytest.raises(
        KeyError,
        match=f"Feature '{not_present_feature}' not found in adata.var_names. Please check the feature name.",
    ):
        GMMThresholding(
            adata=sample_adata,
...
@pytest.mark.parametrize(
    "invalid_label, expected_exception,match_pattern",
    [
...
def test_init_invalid_label_type(
    sample_adata, invalid_label, expected_exception, match_pattern
):
...
def test_init_existing_obs_label(sample_adata):
    ...
    existing_obs_label_adata = create_modified_adata(
        sample_adata, add_obs={label_name: np.repeat(0, len(sample_adata))}
    )  # Ensure the original sample_adata is unchanged

    with pytest.raises(
        KeyError,
        match=f"obs key '{label_name}' already exists in the AnnData object. Please choose a different label.",
    ):
        GMMThresholding(
...
@pytest.mark.parametrize(
    "invalid_kwargs", ["not_a_dict", 123, ["list", "is", "not", "dict"]]
)
