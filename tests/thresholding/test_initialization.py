...
def test_init_existing_uns_key_wrong_type(sample_adata):
    ...
    wrong_uns_type_adata = create_modified_adata(
        sample_adata, add_uns={"gmm_thresholding_events": {"not_dict": "values"}}
    )  # Ensure the original sample_adata is unchanged

    # Match the updated error message in __init__
    with pytest.raises(
        TypeError,
        match="The 'gmm_thresholding_events' key in the AnnData object's `.uns` attribute must be an OrderedDict.",
    ):
        GMMThresholding(
