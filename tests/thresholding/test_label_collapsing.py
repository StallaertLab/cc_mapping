...
def test_collapse_to_binary_automatic_thresholding(sample_gmm_thresholding_instance):
    ...
    assert len(thresholds) == 1, "Should have 1 threshold for 2 categories"

    # Should have condensed labels
    assert gmm._internal_data.condensed_labels == [
        "Low",
        "High",
...
def test_collapse_to_binary_manual_thresholding(sample_gmm_thresholding_instance):
    """Test that duplicate_labels=True raises error with manual thresholds."""
    gmm = sample_gmm_thresholding_instance
    gmm.fit(n_components=4)

    manual_threshold = [1.5]

    # Should raise error when duplicate_labels=True with manual thresholds
    with pytest.raises(
        ValueError, match="duplicate_labels=True cannot be used with manual_thresholds"
    ):
        gmm.categorize_samples(
    ...
    # Should work correctly without duplicate_labels
    gmm.categorize_samples(
    ...
    # Verify labels
    unique_labels = set(gmm.adata.obs["labels"].unique())
    assert unique_labels == {"Low", "High"}, "Should have 2 unique labels"
...
def test_collapse_to_three_categories(sample_gmm_thresholding_instance):
    ...
    thresholds = gmm.return_thresholds()
    assert len(thresholds) == 2, "Should have 2 thresholds for 3 categories"
    ...
    # Should use automatic thresholding
    assert not gmm._manual_decision_boundaries, "Should use automatic thresholding"
...
def test_collapse_many_to_few(sample_gmm_thresholding_instance):
    ...
    gmm.categorize_samples(
        ordered_labels=["Low"] * 3 + ["High"] * 5, duplicate_labels=True
    )
    ...
    assert np.shape(gmm._internal_data.gmm_info.condensed_data_probs)[1] == 2, (
...
def test_collapse_requires_flag(sample_gmm_thresholding_instance):
    ...
    with pytest.raises(ValueError, match="ordered GMM labels contain duplicate values"):
        gmm.categorize_samples(
            ordered_labels=["Low", "Low", "High", "High"],
...
def test_collapse_preserves_n_components(sample_gmm_thresholding_instance):
    ...
    gmm.categorize_samples(
        ordered_labels=["Low"] * 4 + ["High"] * 4, duplicate_labels=True
    )
...
def test_collapse_creates_condensed_probabilities(sample_gmm_thresholding_instance):
    ...
    assert condensed_probs is not None, "Condensed probabilities should be created"
    ...
    assert np.all(max_probs <= 1.0), "Probabilities should not exceed 1.0"
    assert np.all(max_probs > 0.0), "Probabilities should be positive"
...
def test_collapse_rejects_non_contiguous_groups(sample_gmm_thresholding_instance):
    ...
    with pytest.raises(ValueError, match="Non-contiguous duplicate labels detected"):
        gmm.categorize_samples(
...
def test_collapse_validates_threshold_count(sample_gmm_thresholding_instance):
    ...
    with pytest.raises(
        ValueError, match="Number of thresholds.*must be one less than.*unique labels"
    ):
        gmm.categorize_samples(
            ordered_labels=["Low", "High"],
...
def test_collapse_with_all_same_label(sample_gmm_thresholding_instance):
    ...
    gmm.categorize_samples(
        ordered_labels=["Low", "Low", "Low", "Low"], duplicate_labels=True
    )
    ...
    unique_labels = set(gmm.adata.obs["labels"].unique())
    assert unique_labels == {"Low"}, "All samples should have the same label"
